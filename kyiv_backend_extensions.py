...
class IsochroneBandsRequest(BaseModel):
    ...
    include_saved: bool = False  # домішати активні лінії, збережені через /custom-lines
...
def create_extended_app():
    ...
    def _startup():
        ...
        kyiv_lines_store.register(app, state)  # додає /custom-lines* + state["load_saved_lines"]
    ...
    def isochrone_bands_with_hypothetical(req: IsochroneBandsRequest):
        ...
        if req.include_saved and "load_saved_lines" in state:
            lines += [HypotheticalLine(**l) for l in state["load_saved_lines"]()]
        with _graph_mutation_lock:
            added_nodes: List[str] = []
            try:
                augmented_graph, added_nodes = add_hypothetical_lines_to_graph(
                    state["static_graph"], state["street_graph"], lines
                )
        ...
        return {
            "bands": bands_out,
            "n_hypothetical_lines": len([l for l in lines if l.type != "road"]),
    ...
    def accessibility_grid_with_hypothetical(req: AccessibilityGridRequest):
        # ВИПРАВЛЕНО: раніше тут не було ні _graph_mutation_lock, ні
        # remove_hypothetical_lines_from_graph() у finally — гіпотетичні
        # вузли (hypo_*) назавжди лишались у спільному state["static_graph"]
        # і "протікали" в наступні запити (в т.ч. звичайний /isochrone без
        # жодних намальованих ліній), а паралельний запит до
        # /isochrone-bands-with-hypothetical міг зіткнутися з чужими hypo_*
        # вузлами того самого імені. Це і була ймовірна причина, чому
        # бекенд "переставав враховувати" щойно намальовану лінію.
        query_time_s = _query_time_s(req.at)
        lines = list(req.lines)
        if req.include_saved and "load_saved_lines" in state:
            lines += [HypotheticalLine(**l) for l in state["load_saved_lines"]()]
        ...
        result["n_hypothetical_lines"] = len([l for l in lines if l.type != "road"])
