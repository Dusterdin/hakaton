...
# ВИПРАВЛЕНО: раніше тут UNDERGROUND_STOPS_PATH вказував на
# stopsInterchangeUnderground.csv, ніби це список станцій з координатами —
# НІ, це таблиця пересадок метро<->наземний транспорт (from_name/to_name,
# без lat/lon), тому load_rail_like_feed() для метро завжди мовчки повертав
# None. Метро тепер підключається окремо через kyiv_metro_module.py
# (build_and_merge_into у create_app() нижче), який використовує:
#   - timePeriodUnderground.json / timePeriodKyivFunicular.json — реальні
#     погодинні інтервали руху (є, завантажено)
#   - stopsInterchangeUnderground.json — довідкові пересадки (є, завантажено)
#   - координати станцій — приблизні, з фронтенду (kyiv-chas.html), бо
#     офіційний ресурс зі списком станцій+координатами ("underground" /
#     "stopsUnderground" на порталі) в цю сесію завантажений не був.
UNDERGROUND_TIMEPERIOD_PATH = "timePeriodUnderground.json"
UNDERGROUND_STOPS_PATH = "stopsInterchangeUnderground.json"   # довідка про пересадки, НЕ координати станцій

# ВИРІШЕНО (на відміну від метро вище): для електрички завантажено ПОВНИЙ
# комплект — і координати платформ (geojson, реальний Point на кожен code1),
# і сам поїзний розклад із конкретними arrival/departure по кожному потягу.
# Підключається через kyiv_metro_module.build_rail_feed() (kind="rail",
# route_type="2"), без наближень і без approx-позначки.
KYIVCITYEXPRESS_STOPS_PATH = "stopsKyivCityExpress.geojson"       # реальні координати 34 платформ
KYIVCITYEXPRESS_STOPTIMES_PATH = "stopTimesKyivCityExpress.json"  # реальний поїзний розклад, 50 потягів
...
# 11=тролейбус, 3=автобус, 1=метро, 7=фунікулер, 2=залізниця (міська
# електричка). Усі чотири "не-GTFS" види транспорту більше не відсутні
# (див. kyiv_metro_module.py) — build_and_merge_into() з того модуля додає
# їх у той самий feed через ці route_type. Колір "2" узгоджено з тим самим
# фіолетовим, що вже використовує RAIL_LOOP у фронтенді (kyiv-chas.html).
ROUTE_TYPE_COLOR = {"0": "#E8891A", "11": "#33C2D9", "3": "#7F93A8", "1": "#E8434F", "7": "#F26D9E", "2": "#9B59B6"}
ROUTE_TYPE_NAME = {"0": "tram", "11": "trolleybus", "3": "bus", "1": "metro", "7": "funicular", "2": "rail"}
...
def build_routes_geojson(feed: GtfsFeed, drive_graph=None) -> dict:
    ...
    # _split_on_gps_anomalies(max_jump_km=3.0 за замовчуванням) розрахований
    # на ФАЙНОГРАНУЛЯРНІ shapes.txt-трейси трамвая/тролейбуса/автобуса, де
    # сусідні точки трасування — метри-десятки метрів, і будь-який стрибок
    # >3км дійсно означає биту GPS-точку. Для метро/rail/фунікулера "shape"
    # тут — це послідовність СТАНЦІЙ (kyiv_metro_module.py), а не GPS-трейс:
    # реальні перегони міської електрички чи метро легко перевищують 3км
    # (напр. окраїнні перегони "Дарниця - Святошин" — 3-5 км), тож той самий
    # поріг хибно ріже суцільну лінію на десяток дрібних відрізків. Тому для
    # цих видів транспорту поріг суттєво ширший — досить, щоб ловити лише
    # справжні аномалії (стрибок через усе місто), а не звичайні перегони.
    STATION_LEVEL_KINDS = {"metro", "rail", "funicular"}
    STATION_LEVEL_MAX_JUMP_KM = 12.0
    ...
    for route_id, shape_ids in route_shapes.items():
        ...
        max_jump_km = STATION_LEVEL_MAX_JUMP_KM if kind in STATION_LEVEL_KINDS else 3.0
        for shape_id in shape_ids:
            pts = feed.shapes.get(shape_id, [])
            if len(pts) < 2:
                continue
            clean_segments = _split_on_gps_anomalies(pts, max_jump_km=max_jump_km)
...
def create_app():
    ...
    def _startup():
        ...
        # Метро + фунікулер + електричка — окремий модуль (не GTFS-архів,
        # див. kyiv_metro_module.py щодо джерела даних кожного виду й
        # відомих обмежень геометрії станцій метро). Якщо файлів немає
        # поруч — пропускається, сервер не падає (як і з основним GTFS вище).
        try:
            import kyiv_metro_module
            state["feed"] = kyiv_metro_module.build_and_merge_into(
                state["feed"],
                underground_timeperiod_path=UNDERGROUND_TIMEPERIOD_PATH,
                funicular_timeperiod_path="timePeriodKyivFunicular.json",
                interchange_path=UNDERGROUND_STOPS_PATH,
                rail_stops_path=KYIVCITYEXPRESS_STOPS_PATH,
                rail_stoptimes_path=KYIVCITYEXPRESS_STOPTIMES_PATH,
            )
            state["stop_node"] = add_transit_stops_and_transfers(
                state["static_graph"], state["street_graph"], state["feed"]
            )
            state["dep_index"] = build_stop_departure_index(state["feed"])
            print(f"[ready] метро+фунікулер+електричка долучені: {len(state['feed'].stops)} зупинок разом")
        except FileNotFoundError as e:
            print(f"[warn] метро/фунікулер/електричка пропущено — не знайдено файл: {e}")
        except Exception as e:
            print(f"[warn] метро/фунікулер/електричка пропущено через помилку: {e}")
